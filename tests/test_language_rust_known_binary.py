...
VERSIONS_64 = pathlib.Path(__file__).resolve().parent / "data" / "language" / "rust" / "rust-binaries-all-versions" / "bin" / "versions_64.zip"
VERSIONS_32 = pathlib.Path(__file__).resolve().parent / "data" / "language" / "rust" / "rust-binaries-all-versions" / "bin" / "versions_32.zip"

if not VERSIONS_64.exists() or not VERSIONS_32.exists():
    pytest.skip("Rust test binaries archive missing", allow_module_level=True)
